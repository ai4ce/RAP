...
names = ["shop", "hospital", "college", "church", "court"]
for name in names:
    TASKS.append((f"Cam_{name}_ref", f"data/Cambridge/{name}/colmap/undistorted", "Colmap", f"{LOGS_DIR}/Cambridge/{name}.pth",
                  f"output/Cambridge/{name}{'' if name == 'court' else '_undist'}", ""))
...
def main():
    ...
    for _ in GPU_CPU_MAP:
        task_queue.put(None)

    processes = []
    for gpu_id in GPU_CPU_MAP:
